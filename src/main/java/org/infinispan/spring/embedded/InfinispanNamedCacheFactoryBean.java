 ...
 * with what <code>Configuration</code> to start before further customizing it as described above:
 * <ul>
 * <li>
 * <code>NONE</code>: Configuration starts with a new <code>Configuration</code> instance. Subsequently,
 * its settings are overridden with those properties that have been explicitly set on this
 * <code>FactoryBean</code>. Note that this mode may only be used if no named configuration having the
 * same name as the <code>Cache</code> to be created already exists. It is therefore illegal to use this
 * mode to create a <code>Cache</code> named, say, &quot;cacheName&quot; if the configuration file used
 * to configure the <code>EmbeddedCacheManager</code> contains a configuration section named 
 * &quot;cacheName&quot;. 
 * </li>
 * <li>
 * <code>DEFAULT</code>: Configuration starts with the <code>EmbeddedCacheManager</code>'s <em>default</em> 
 * <code>Configuration</code> instance, i.e. the configuration settings defined in its configuration file's
 * default section. Subsequently, its settings are overridden with those properties that have been explicitly
 * set on this <code>FactoryBean</code>. Note that this mode may only be used if no named configuration having the
 * same name as the <code>Cache</code> to be created already exists. It is therefore illegal to use this
 * mode to create a <code>Cache</code> named, say, &quot;cacheName&quot; if the configuration file used
 * to configure the <code>EmbeddedCacheManager</code> contains a configuration section named 
 * &quot;cacheName&quot;. 
 * </li>
 * <li>
 * <code>NAMED</code>: Configuration starts with the <code>EmbeddedCacheManager</code>'s <code>Configuration</code>
 * instance having the same name as the <code>Cache</code> to be created. For a <code>Cache</code> named, say, 
...
public class InfinispanNamedCacheFactoryBean implements FactoryBean<Cache<Object, Object>>, BeanNameAware,
	...
	private Cache<Object, Object> configureAndCreateNamedCache(final String cacheName) {
		...
		case NONE:
			...
			if (this.infinispanEmbeddedCacheManager.getCacheNames().contains(cacheName)) {
				throw new IllegalStateException("Cannot use ConfigurationTemplateMode NONE: a cache named ["
						+ cacheName + "] has already been defined.");
			}
			final Configuration newConfiguration = new Configuration();
			final Configuration customizedNewConfiguration = this.infinispanEmbeddedCacheManager.defineConfiguration(
					cacheName, newConfiguration);
		...
		case NAMED:
			...
			final Configuration customizedNamedConfiguration = this.infinispanEmbeddedCacheManager.defineConfiguration(
					cacheName, cacheName, namedConfiguration);
		...
		case DEFAULT:
			...
			if (this.infinispanEmbeddedCacheManager.getCacheNames().contains(cacheName)) {
				throw new IllegalStateException("Cannot use ConfigurationTemplateMode DEFAULT: a cache named ["
						+ cacheName + "] has already been defined.");
			}
			final Configuration defaultConfiguration = this.infinispanEmbeddedCacheManager.getDefaultConfiguration()
					.clone();
	...
	/**
	 * <p>
	 * Sets the {@link org.infinispan.manager.EmbeddedCacheManager <code>org.infinispan.manager.EmbeddedCacheManager</code>} to 
	 ...
	 * 			The {@link org.infinispan.manager.EmbeddedCacheManager <code>org.infinispan.manager.EmbeddedCacheManager</code>} to
